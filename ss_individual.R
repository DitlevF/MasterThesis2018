...
ss_individual <- function(X, Y, fix = rep(0,ncol(X)), a_omega_start, b_omega_start, c = 1, iter = 1000, print_iter = TRUE){
  ...
  # Prior for a_omega and b_omega
  a_omega <- if (missing(a_omega_start)) rep(0.5,k) else a_omega_start # If not specified, assign a fixed prior
  b_omega <- if (missing(b_omega_start)) rep(0.5,k) else b_omega_start
  ...
  # Initialize delta, omega and beta
  delta <- rep(FALSE,k)
  beta <- rep(0,k)
  omega <- rbeta(k,a_omega,b_omega) # Initial guess based on the prior
  ...
  for(j in 1:iter){ 
    ...
    # Step II: omega
    omega <- rbeta(k,delta + a_omega, 1 + b_omega - delta)
    omega_draws[j,] <- omega 
  ...
  draws <- list('betas' = data.frame('betas' = beta_draws), 'deltas' = data.frame('deltas' = delta_draws), 'mu' = mu_draws, 'sigma2' = sigma_draws, 
                'omegas' = data.frame('omegas' = omega_draws), 'Rj' = data.frame('Rj' = Rj_draws), 'post_delta' = data.frame('post_delta' = post_delta_draws))
