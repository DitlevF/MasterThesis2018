...
ss_NMIG <- function(X,Y, r = 1/10000, iter = 1000, print_iter = TRUE){
  ...
  for(i in 1:iter){
    ...
    for(j in 1:k){
      ...
      # 2a: Deltas
      Lj <- dt_ls(beta[j], df = 2*v, mean = 0, sd = sqrt(r*Q/v)) / dt_ls(beta[j], df = 2*v, mean = 0, sd = sqrt(Q/v))
      ...
      # 2b: Psis
      psi[j] <- 1/rgamma(1, shape = v + 0.5, rate = Q + beta[j]^2/(2*rdelta[j]))
    }
    ...
    beta <- rmvnorm(1, mean = aN, sigma = AN)
    beta_draws[i,] <- beta
    
    # Step 5: Sample sigmas
    sN <- (N-1)/2
    SN <- 0.5 * t((yc - X %*% t(beta))) %*% (yc - X %*% t(beta))
