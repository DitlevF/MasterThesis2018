...
###### Model 1 ######
#####################
x <- "efaviol + xxprison + xxpolice + xxunemp + xxincome + xxpover + xxafdc15 + xxgunlaw + xxbeer + factor(year)"  
y <- "lpc_viol"
z <- paste(y,"~",x)

model1 <- plm(formula = as.formula(z), data = df, model = "fd")

# Compute Clustered SE's
G <- length(unique(df$statenum)) # Number of groups, 50 states
N <- length(df$statenum) - 50 # Only 600 used for estimation
dfa <- (G/(G-1)) * (N-1) / model1$df.residual # Degrees of freedom

state_c_vcov <- dfa * vcovHC(model1, type = "HC0", cluster = "group")

summary(model1, vcov = state_c_vcov )

#####################
###### Model 2 ######
#####################
x <- "efaprop + xxprison + xxpolice + xxunemp + xxincome + xxpover + xxafdc15 + xxgunlaw + xxbeer + factor(year)"  
y <- "lpc_prop"
z <- paste(y, "~", x)

model2 <- plm(formula = as.formula(z), data = df, model = "fd")
state_c_vcov <- dfa * vcovHC(model2, type = "HC0", cluster = "group")

summary(model2, vcov = state_c_vcov )

#####################
###### Model 3 ######
#####################
x <- "efamurd + xxprison + xxpolice + xxunemp + xxincome + xxpover + xxafdc15 + xxgunlaw + xxbeer + factor(year)"  
y <- "lpc_murd"
z <- paste(y, "~", x)

model3 <- plm(formula = as.formula(z), data = df, model = "fd")
state_c_vcov <- dfa * vcovHC(model3, type = "HC0", cluster = "group")

summary(model3, vcov = state_c_vcov )
...
for(i in xx){
  ...
  df <- df %>%
    group_by(statenum) %>%
    mutate(z = get(i) - lag(get(i))) # mutate(group_by(df, statenum), z = get(i) - lag(get(i))) # Alternative formulation . Get() transform string to variable
...
#names(df)[names(df) == 'z'] <- new_col
#Dxx02 <- cbind(Dxx02,new_col)

#}
Dxx02 <- c(Dxx02)[-1]

# Create Big List
biglist <- c(xx_d,xx_d_sq,xx_d_int,Lxx,Lxx2,Mxx,Mxx2,xx0,xx02,Dxx0,Dxx02)
